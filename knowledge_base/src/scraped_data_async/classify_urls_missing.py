...
# -*- coding: utf-8 -*-
"""
classify_urls.py   ·   Categorise Nestlé pages (Recipe / Product / …)

变动：
1. url2file 从 *.html 里直接解析 canonical / og:url，避免文件名对不上 URL。
2. load_page_snippet() 兼容 list 和 dict 两种 _text.json 格式，防止 KeyError。
3. 其它逻辑沿用：Regex → LLM(正文+URL) → 输出 classified_urls.jsonl → 写回 metadata
"""
...
# ─────────────────────────── config ────────────────────────────
DATA_DIR    = Path("scraped_data_retry")
